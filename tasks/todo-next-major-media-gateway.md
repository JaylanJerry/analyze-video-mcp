...
| 项                                              | 结论                     | 处置                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ----------------------------------------------- | ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F1 [P1] 回答漏脱敏部分绝对路径                  | **确认，已修**           | 已复现（`C:/…`、`/tmp/…`、`/var/tmp/…` 原样穿出）。`src/sanitize.ts` 现在同时覆盖反斜杠/正斜杠盘符与一般 POSIX 绝对路径，并保留 URL、时间码与相对路径；新增 `test/sanitize.test.ts`（7 例，含“不吃 URL 路径”“URL 与时间码不变”反例）                                                                                                                                                                                                            |
| F2 [P1] 缓存不识别同路径同大小同 mtime 的新内容 | **确认，已修**           | `AuthorizedLocalMedia` 新增 `contentFingerprint`（大小 + 首尾各 64 KiB 的 SHA-256 前缀，读自同一只读句柄），缓存键纳入该指纹；新增 `test/media-identity.test.ts` 的“同路径同大小同 mtime、内容改变 → 指纹不同”与 `test/upload-cache.test.ts` 的“指纹变化即 miss”。**残余（已声明）**：只改中段且保持大小与 mtime 不变的就地改写不会被发现，与既有的同账户 TOCTOU 残余同级——按审核建议的备选（对不确定项禁用缓存）会牺牲既有的上传复用，故未采用 |
| F3 [P1] MP3 仅凭前 16 帧断言恒定码率            | **确认，已修**           | 已复现（前 16 帧 128 kbps + 后续 100 帧 32 kbps → 报 1.067 秒，真实 3.030 秒）。改为**跨全文件多点抽样**（0.99/0.25/0.5/0.75/0.95/0.9/0.8 各取窗口比对首帧，任一点不同即不判定 CBR），无法证明时返回未知时长；新增 3 例回归（开头 CBR 后续 VBR、末段变码率、尾部标签仍可估算），并把“一小时”用例改为 Xing 帧数声明的可靠时长（含“正好 3600 允许”边界）                                                                                          |
