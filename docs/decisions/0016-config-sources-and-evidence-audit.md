...
- 成功结果出口同样执行路径、凭证形态与内部 `oss://` 脱敏，涵盖正文和结构化观察，避免模型复述内部值造成泄露。
- 散文回答不产生分项“已核实”语义；结构化报告的声音正文确定性结论须有 `heard` 条目支持。时间码检查 `MM:SS` 格式以及已知本地时长边界；无效条目降为不确定内容。
- `finish_reason=length` 不再按成功文本返回；安全诊断携带截断原因、事件数和可用 token 用量，不保留 provider 原始响应。
