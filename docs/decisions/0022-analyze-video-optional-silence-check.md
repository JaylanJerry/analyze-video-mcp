...
2026-09-25 桌面后续真实调用发现阴性 `heard` 新措辞变体：20 秒全零 AAC 对照（SHA256 `8DA8D6D432C246686324977C96D70FE591C60E598554C7D88BBA65E3796E768E`）在 1 次 `qwen3.5-omni-plus` 调用中返回 `heard` 描述“音轨全程为静音状态，未检测到任何背景音乐、对白或音效。”，当时被误报为 1 项 `evidence_conflicts`。本地修正规则仅新增精确短语“音轨全程为静音状态”作为阴性分句锚点；每个分句仍须明确否定，任何未明确否定分句继续冲突，包括混合正向声音和“音轨不为静音状态”反例。默认 off、非静音、未测和 HTTPS 路径不受影响。桌面 live 观察只用于复现缺陷；没有新付费调用验证修复，mock 回归不等同 live 验收。

2026-09-25 桌面后续复测与文案修复：同一 20 秒全零 AAC 先后发起 2 次真实 `qwen3.5-omni-plus` 调用：首轮返回可重试 `data_inspection_failed`，第二次成功，报告 `audio_track_present=true`、`audio_observed=false`、`audio_observations=[]`、无 conflict、PCM 全零；两次调用可能产生费用，具体计费未知。模型没有返回 `heard` 阴性描述，因此未 live 验证上一条规范化修正。新模型输出把“整个视频音轨为完全静音，未检测到任何可辨识的声音内容，包括对白、背景音乐、环境噪音或音效”标成 `无效时间码`，并推断相关声音元素“实际并未录制或混入音频”。仅在确认数字静音分支中，新增对这一条完整全片静音 uncertainty 的精确过滤，并把该条来源推断修为当前文件音轨 PCM 全零、原始录音或后期混音原因未知；局部音频或真实视觉时间码错误保留，合理视觉推断保留。mock 覆盖真实完整文本和反例，五项质量门通过（`npm test`: 309 passed / 12 skipped），pack-install smoke 通过；未追加用于验证本轮修复的 provider 调用。

同次结果另称“无法判断该视频是否原本设计为有声版本，或因技术原因导致音轨丢失”。有已探测 AAC 音轨不代表已知它为何全零，也不能排除历史制作或转码原因；故本地只把该精确句子改为“无法判断该视频是否原本设计为有声版本，或现有音轨为何全零”，不泛化改写其他不确定性。该修正仅由已确认有轨且完整数字静音的路径触发。
