...
- 现在不加图片、独立音频 Tool。
- 不从本机主动发 npm、不推送，除非用户另说。已授权的 `v*` tag 由 `release.yml` 发 npm。
...
- 模板用 `npx -y analyze-video-mcp`。GitHub 回退钉 `#v0.5.0`，并带 `--allow-git=all`（npm 12）。跟最新提交用 `#main`。普通人只填 Key 就能分析本地绝对 MP4。建议填写 `QWEN_ALLOWED_ROOTS`。
...
- 质量门保持现有门槛。
- 已发 npm `analyze-video-mcp`。默认安装不再钉 GitHub 标签。
