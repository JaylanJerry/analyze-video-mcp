...
| 项                                                   | 必需 | 说明                                                                        |
| ---------------------------------------------------- | ---- | --------------------------------------------------------------------------- |
| `npx -y github:JaylanJerry/analyze-video-mcp#v0.5.0` | 是   | 默认钉发布标签。跟最新提交用 `#main`。不发 npm。                            |
...
- 模板用 `npx`，默认钉 `#v0.5.0`；跟最新提交用 `#main`。普通人只填 Key 就能分析本地绝对 MP4。建议填写 `QWEN_ALLOWED_ROOTS`。
