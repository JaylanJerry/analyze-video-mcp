...
- 有界探测：跳过 ID3v2（声明长度超界即拒绝）、在有限窗口内找首帧、验证连续帧。
- 只读帧头与 Xing/Info/VBRI 头部，不做全文件扫描、不缓冲整文件、不依赖任何解码器。
- 时长只在**流自己声明**时给出（Xing/Info/VBRI 帧数），并做合理性校验：声明帧数与文件大小矛盾时不产出数字。不做码率估算——采样无法证明整个文件是恒定码率（采样点之间的区段可能不同），而错误数字会驱动一小时门禁，因此没有声明帧数就报未知。
