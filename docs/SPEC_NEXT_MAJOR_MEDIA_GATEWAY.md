...
| 项目       | 下一大版本目标                                                   | 验证边界                                                                                                       |
| ---------- | ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| Tool 表面  | 恰好一个核心 Tool：`analyze_media(media, prompt)`；`prompt` 必填 | 先以契约测试冻结，再实施；不在当前版悄悄改 `analyze_video`                                                     |
| 包/启动    | 沿用 `analyze-video-mcp` 包名、CLI 与现有 Host 配置键            | 发布前核对安装和握手；包名不代表只接受视频                                                                     |
...
上表描述下一大版本目标及当前工作区的实现与验证边界，不表示 npm 已发布版本支持 MP3 或新 Tool。技术限制若与目标冲突，先记录证据并调整规格，不用未经验证的隐式转码或另一家服务商掩盖失败。
...
本规格替代旧 v0.7 提案中“三 Tool + 确定性测量 + 综合审核”的**产品方向**，但不抹去已实现和已接受的安全修复。历史提案见 [`SPEC_V07_PROPOSAL.md`](SPEC_V07_PROPOSAL.md)、ADR 0018/0019；已发布 `0.6.1` 的数字静音可选能力见 ADR 0022。当前工作区下一大版本已移除该 FFmpeg 静音测量及其对回答的语义改写，迁移表说明 `QWEN_AUDIO_SILENCE_CHECK` 不再生效；数字静音事实若将来仍是独立需求，再另立功能规格。当前分支的 `API_CONTRACT.md`、`ARCHITECTURE.md`、`PROVIDER_PROTOCOL.md`、`SECURITY.md`、README、doctor、Server instructions 和 Tool 描述已随实现更新，发布前仍须核对验收与最终版本文案；本规格不能被当作已发布 npm 包的行为说明。
