...
> **给独立审核者的入口（2026-09-25）：** 先读 [`tasks/archive/1.0/codex-review-brief-20260925.md`](../../../tasks/archive/1.0/codex-review-brief-20260925.md)——它自带来审核目标、代码地图、复现命令、逐项检查清单、已知边界与「不要做」红线，并区分 mock / 真实百炼 / 宿主 GUI / 打包安装 / npm 发布五种状态。
>
> **2026-09-25 下一大版本接手入口：** 用户已确认“一个媒体分析 Tool、Agent 决定问题、本地 MP4/MOV/MP3、首发百炼”的大版本重构方向。DeepSeek 开发请先读 [`docs/SPEC_MEDIA_GATEWAY.md`](../../SPEC_MEDIA_GATEWAY.md)、[ADR 0024](../../decisions/0024-agent-directed-media-gateway.md)、[`tasks/archive/1.0/plan-next-major-media-gateway.md`](../../../tasks/archive/1.0/plan-next-major-media-gateway.md)、[`tasks/archive/1.0/todo-next-major-media-gateway.md`](../../../tasks/archive/1.0/todo-next-major-media-gateway.md) 和 [`tasks/archive/1.0/deepseek-next-major-handoff.md`](../../../tasks/archive/1.0/deepseek-next-major-handoff.md)。以下 v0.7 三 Tool/综合审核计划是历史基线，**不再作为下一大版本实施指令**。当前工作区分支已实现 `analyze_media` 并**本地提交 `381da84`**（未推送、未发布）；npm 上的 `0.6.1` 仍使用 `analyze_video`。

状态：v0.6.1 已在 npm（[`docs/archive/specs/SPEC_V061.md`](../specs/SPEC_V061.md)、[ADR 0016](../../decisions/0016-config-sources-and-evidence-audit.md)）。v0.6.0 基线见 [`docs/archive/specs/SPEC_V06.md`](../specs/SPEC_V06.md)，未单独打 tag。推已授权的 `v*` tag 时，`release.yml` 用 Trusted Publishing 发 npm 并建 GitHub Release（[ADR 0014](../../decisions/0014-npm-trusted-publishing.md)）。人须在 npm 包设置里点一次 Trusted Publisher。不要添加 `NPM_TOKEN`。不要补打已发 npm 的 `v0.5.1` / `v0.5.2`，也不要补打 `v0.6.0`。
...
**2026-09-26 Luna 用户追加测试排查：** 新任务手动拖入已正确转路径并选择 `analyze_media`，但指定样本授权后上传失败，旧结果无诊断，根因未知。另一份 1455 秒 MP4 两次分析成功，上传复用 `false → true`。原始结果两份完整正文仍在任务记录，截断直接发生在 Agent 中间执行层打印重复答案时；已本地恢复原文并校验分段拼接。当前改动补长结果交付指引和 multipart 安全诊断，不宣称历史上传失败已修、不宣称 GUI 已执行新指引。见 [`tasks/archive/1.0/codex-acceptance-20260926.md`](../../../tasks/archive/1.0/codex-acceptance-20260926.md) 新增排查节。没有新增付费调用；Node 22/24 PR CI、推送和发布仍未进行。

**2026-09-26 Codex 重启后复验：** 用户重启 Codex 后明确要求直接测试，同一合成 MOV/MP3 各调用一次均成功（`qwen3.5-omni-plus`，usage 分别 769/26/795、151/77/228）。MCP 进程启动晚于本地构建，全局入口指向本仓库且关键 `dist` 哈希相同；最新构建经新进程加载的调用已通过，覆盖下方该项“待验”的旧状态。**新会话手动拖入与 Node 22/24 PR CI 仍待验**。完整记录见 [`tasks/archive/1.0/codex-acceptance-20260926.md`](../../../tasks/archive/1.0/codex-acceptance-20260926.md)。

**2026-09-26 独立收口更新（覆盖下方 2026-09-25 的“Codex 宿主 MOV/MP3 待验”状态）：** 当前 Codex 任务已在用户明确授权下，用合成 3 秒 MOV 与 9 秒 MP3 各调用一次 `analyze_media`，均成功并给出与夹具一致的回答；加上先前公开 MP4，当前任务的三种本地格式均有宿主调用证据。新增的上传成功/取消竞态与 Windows 带空格路径打包验收脚本问题已修，本地 Node 24 门禁 297 passed / 1 skipped，带空格路径的打包安装及握手通过。完整证据见 [`tasks/archive/1.0/codex-acceptance-20260926.md`](../../../tasks/archive/1.0/codex-acceptance-20260926.md)。**仍待** Codex 新会话手动拖入与新进程加载最新构建；Node 22/24 选择稍后走 PR CI，当前分支普通 push 不触发 CI。两次新调用的账单金额未知，不把模型回答视为内部模态路径证明。未推送、未发布。

**2026-09-25 Codex 宿主补验：** 当前 Codex 任务已挂载 `mcp__analyze_video_mcp__analyze_media`，并用仓库公开 3 秒 MP4 夹具完成一次真实调用；`qwen3.5-omni-plus` 报告看到 `24`、听到 `3.1415926`，与夹具说明一致，`isError=false`、`usage=770/26/796`。文件前后 SHA-256 相同。此项验证当前任务的 Tool 调用链；Codex 新会话拖入与 Codex 宿主 MOV/MP3 仍未验证。该次服务商费用金额未知。详情见 [`tasks/archive/1.0/todo-next-major-media-gateway.md`](../../../tasks/archive/1.0/todo-next-major-media-gateway.md) D4。

**2026-09-25 下一大版本已在本工作区分支实现（已本地提交 `381da84`，未推送、未发布）：** 按 [`docs/SPEC_MEDIA_GATEWAY.md`](../../SPEC_MEDIA_GATEWAY.md) 与 [ADR 0024](../../decisions/0024-agent-directed-media-gateway.md) 完成单入口 `analyze_media(media, prompt)`：本地 MP4/MOV **与 MP3**、公开 HTTPS 视频直连；服务端不再补写九段提纲、不再强制证据 JSON、不再有纠错二次请求；本地授权改为 `MEDIA_ALLOWED_ROOTS` / `MEDIA_ALLOW_ANY_LOCAL_FILE`（旧 `QWEN_ALLOWED_ROOTS` / `QWEN_ALLOW_ANY_LOCAL_VIDEO` / `QWEN_MAX_LOCAL_VIDEO_MB` / `QWEN_AUDIO_SILENCE_CHECK` 只被 doctor 报告为失效，不授予任何访问）；旧报告层 `src/evidence.ts`、可选 FFmpeg 静音核对 `src/audio-silence.ts`、`analyze_video` 注册与旧报告专用测试已删除（内容仍在 git 历史）。新增 `src/mpeg-audio.ts`（有界 MPEG Layer III 解析）、`src/bytes.ts`（共享探测预算）、`src/sanitize.ts`（保留的脱敏出口）；上传缓存键加入 API Key 单向指纹，换账号不再复用旧 `oss://`。本地门禁全通过：typecheck / lint / format:check / `npm test` 258 passed+1 skipped / coverage 88.18% stmts、81.84% branch / build；`dist/index.js --doctor` 只注册 `analyze_media`。**2026-09-25 真实百炼验收（用户授权，共 7 次调用）：** ① 非私密合成 MP3（9.04 秒、三段递增音调）两次调用都成功，模型准确说出三段与“依次升高”，第二次 `upload_reused:true`；② 890 秒真实 MP3 成功概括内容；③ 公开夹具 MP4 与 `-c copy` 转封装的 MOV 都读出画面 `24` 与语音 `3.1415926`。**负例：** 只有音频轨、没有视频轨的 MP4 经视频路径被服务商 **HTTP 400** 拒绝（复现 2 次，原因未定位）。全部运行的 stderr 与正文均无 `oss://`、密钥或本地路径。**ZCode 重装（2026-09-25）：** `~/.zcode/cli/config.json` 里指向已删除目录的旧 `analyze_video_mcp` 已整条删除并重新注册为当前仓库（env 用 `MEDIA_*`，`enabled=true`，备份 `.bak-before-reinstall`），并用与 ZCode 相同的 node/args/cwd/env 完成真实调用：MP4 夹具画面 `24` + 语音 `3.1415926`、合成 MP3 两次均正确（三段、低→中→高）。**ZCode GUI 新会话已确认一次（2026-09-25）：** 用户在该会话拖入 `live-av.mp4` 的 `-c copy` MOV 转封装（本地核对 `mdat` 与公开夹具逐字节一致），`analyze_media` 真实调用成功，画面 `24` + 语音 `3.1415926`、`usage` 904/3212/4116、`upload_reused=true`、`request.model=qwen3.8-omni-flash`；同一会话又拖入 §3b 那份 72,559 B MP3 样本并通过（`media.kind=audio`、`usage` 274/1522/1796），但模型把 3 段音调报成 4 段、切换点报成 2/5/8 秒（本地 `aspectralstats=centroid` 实测为 3 段、≈2.9/5.9 秒）——宿主可用，模型计数不可信。**仍属未验证：** `MEDIA_MODEL_UNSUPPORTED` 的服务商真实措辞、Codex 新会话拖入与宿主 MOV/MP3、费用金额、内容检查拒绝的输入/输出侧别判定；Node 22 与 CI 未跑（本机只有 Node 24，且没有推送）。详见 [`tasks/archive/1.0/todo-next-major-media-gateway.md`](../../../tasks/archive/1.0/todo-next-major-media-gateway.md)。npm 上的 `0.6.1` 未改动，仍是 `analyze_video`；本轮已**本地提交 `381da84`**（66 文件），但没有 push、tag 或 publish。
...
2026-09-22 后续优化交接：用户指定先处理 Codex 中“手动拖入任意文件夹视频”的体验，再按音频和综合审核路线开发。接手步骤、无付费 Codex 探针与分阶段验收见 [`tasks/archive/pre-1.0/deepseek-v07-handoff.md`](../../../tasks/archive/pre-1.0/deepseek-v07-handoff.md)。A 批代码已落地（默认行为未变），B 批按“先复现再修”进行。

2026-09-22 拖入体验结论：A1 探针证明 Codex 只把拖入视频写成模型可读的路径文本，MCP 边界拿不到可验证附件，用户据此否决确认弹窗，选定安装级开关 `QWEN_ALLOW_ANY_LOCAL_VIDEO`（默认 `off`；`on` 时任意本地受支持视频路径直接上传，路径即授权）。决策与代价见 [ADR 0021](../../decisions/0021-allow-any-local-video-opt-in.md)。状态分层见 [`tasks/archive/pre-1.0/todo-v07-proposal.md`](../../../tasks/archive/pre-1.0/todo-v07-proposal.md) 与 [`tasks/archive/pre-1.0/deepseek-v07-handoff.md`](../../../tasks/archive/pre-1.0/deepseek-v07-handoff.md)：代码与 CLI 开关验证已完成；用户授权后的真实 MOV 脚本调用、MP4 MCP 工具调用已成功。**2026-09-23 Codex 桌面 GUI 已用公开 3 秒夹具完成一次拖入 → MCP 调用 → 画面及语音数字正确返回，用户报告未出现批准或风险审查提示**；较大文件和其它目录的 GUI 情形未验证。一次真实 MOV 调用在取凭证阶段返回 `request_failed`、重试成功，失败原因仍未知；《AE海-通义.mp4》模型回答未确认音频，本地与受控替换音轨的对照证据见协议文档。该分支尚未进入已发布版本。

绑定文档：

1. [`docs/archive/specs/SPEC_V061.md`](../specs/SPEC_V061.md)
2. [`docs/decisions/0016-config-sources-and-evidence-audit.md`](../../decisions/0016-config-sources-and-evidence-audit.md)
3. [`docs/archive/specs/SPEC_V06.md`](../specs/SPEC_V06.md)
4. [`docs/decisions/0015-host-reliability-and-evidence-gate.md`](../../decisions/0015-host-reliability-and-evidence-gate.md)
5. [`docs/decisions/0014-npm-trusted-publishing.md`](../../decisions/0014-npm-trusted-publishing.md)
6. [`docs/archive/specs/SPEC_V07.md`](../specs/SPEC_V07.md)
7. [`docs/decisions/0017-host-config-key-analyze-video-mcp.md`](../../decisions/0017-host-config-key-analyze-video-mcp.md)
...
下一大版本先按本文开头的五份目标文档读，再核对 `AGENTS.md`、`docs/API_CONTRACT.md`、架构 / 协议 / 安全 / 测试及现行代码。`docs/archive/specs/SPEC.md` 与 `tasks/archive/pre-1.0/todo.md` 只用于追溯 v1 背景。
