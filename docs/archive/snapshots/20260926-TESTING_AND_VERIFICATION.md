...
> **当前下一大版本运行时策略（2026-09-26，见 [ADR 0025](../../decisions/0025-next-major-node24-support.md)）：** 本分支只正式支持 Node 24.x；Windows 本地验收使用 Node 24，远程 Node 24 CI 仍待验证。下文 Windows 验收矩阵属于已发布 v1 的历史门槛，其中 Node 22 的要求不再适用于未发布的下一大版本；npm `0.6.1` 原有声明与历史记录不变。
