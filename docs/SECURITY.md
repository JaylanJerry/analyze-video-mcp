...
> 本文件描述**已正式发布的 1.0.0**：`analyze_media(media, prompt)`。旧 `analyze_video` 授权变量不再生效，见下方迁移说明。
...
- Agent 可见的回答与结构化字段在单一出口做**两层**路径脱敏：① 本次调用 Agent 传入的确切路径（含两种斜杠风格与 Windows 大小写变体）按字面替换，因此文件名里的空格、假名、标点都能覆盖；② 通用规则作为补充，覆盖本次未传入的 Windows 盘符/UNC（两种斜杠）、前斜杠 UNC 与 POSIX 路径（含非 ASCII 段）。普通 HTTPS 链接、时间码与「画面/声音」这类散文保留；确定性替换**不会改写公开 http(s) 链接内部的匹配**（本地路径可能恰好是某个公开链接的子串，改写它会破坏链接）。当句读标点紧接本次传入的路径时，标点结束链接区段，后面的路径仍须完整隐藏。**界限**：通用规则不跨空白，因此“本次未传入、且文件名含空格”的路径可能只被部分脱敏；已传入的路径由第一层完整覆盖。
...
- 取消与上传成功同时发生时，即使底层上传器返回了 `oss://`，已取消的调用也不创建新的缓存项；若取消落在磁盘持久化期间，清除本进程缓存并尽力改写磁盘缓存。调用边界仍将该次操作报告为 `MEDIA_ANALYSIS_CANCELLED`。这条竞态由 `test/upload-cache.test.ts` 的内存与磁盘回归覆盖。
